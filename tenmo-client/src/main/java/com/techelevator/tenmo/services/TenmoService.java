...
public class TenmoService {
    ...
    public String requestBucks (AuthenticatedUser user ,int requestUserId, BigDecimal amount){
      try {
          ...
          transfer.setNameOfTransferType("Request");
          ResponseEntity<Transfer> response = restTemplate.postForEntity(baseUrl + "transfer/request"
                  , makeTransferEntity(user,transfer), Transfer.class);
          return response.getBody().getNameOfTransferStatus();
    ...
    public String updateTransfer(int transferStatus, int transferId, AuthenticatedUser currentUser) {
        try{

            ResponseEntity<Integer> response = restTemplate.exchange(baseUrl + "transfer/" + transferId + "/" + transferStatus,
                    HttpMethod.PUT, makeGetEntity(currentUser), Integer.class);
            response.getBody();
            if(response.getBody().equals(1)) {
              return "Success!";
            } else {
                return "It is past the due time and I am not sure what else to put here";
            }
        } catch (RestClientResponseException | ResourceAccessException ex) {
            BasicLogger.log(ex.getMessage());
            return ex.getMessage();
