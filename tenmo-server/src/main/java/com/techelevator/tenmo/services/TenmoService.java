...
public class TenmoService {
    ...
    public Transfer requestBucks(Transfer transfer, String verifiedName) {
        if(verifiedName.equals(transfer.getToName())) {
            int accountToId = accountDao.getAccountIdByUserId(transfer.getUserIdTo());
            try {
                int accountFromId = accountDao.getAccountIdByUserId(transfer.getUserIdFrom());
                return transferDao.createTransfer(transfer, accountFromId, accountToId);
            } catch (Exception ex) {
                throw new IdNotFoundException("Please enter a valid ID");
            }
        }  else {
    ...
    public int updateTransfer(int id, int status, String currentUser) {
        try {
            if (status == 1) {
                Transfer approvedTransfer = transferDao.getTransferById(id);
                int userIdTo = accountDao.getUserIdByAccountId(approvedTransfer.getAccountTo());
                int userIdFrom = accountDao.getUserIdByAccountId(approvedTransfer.getAccountFrom());
                accountDao.addBalance(approvedTransfer.getAmount(), userIdTo);
                accountDao.subtractBalance(approvedTransfer.getAmount(), userIdFrom);
                return transferDao.updateTransfer(id, 2);
            } else if (status == 2) {
                return transferDao.updateTransfer(id, 3);
            } else {
                return 0;
            }
        } catch(Exception ex) {
            throw new IdNotFoundException("Please try again; check your inputs closely");
        }
    }
}
