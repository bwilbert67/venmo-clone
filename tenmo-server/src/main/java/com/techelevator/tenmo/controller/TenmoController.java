...
public class TenmoController {
    ...
    public Transfer requestBucks(@RequestBody Transfer transfer, Principal principal) {
        return tenmoService.requestBucks(transfer, principal.getName());
    }
    @PutMapping("transfer/{id}/{status}")
    public int updateTransfer(@PathVariable int id, @PathVariable int status, Principal principal) {
        return tenmoService.updateTransfer(id, status, principal.getName());
    }
}
